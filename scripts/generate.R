...
# `format(x, digits = 7)` and `proc.time() - ...` when timing examples,
# pkgdown's site build reaches `length()` through `purrr::pluck()` while
# rendering the navbar, and roxygen2's CI wrapper counts its own warnings with
# `length()`. A stub of that name on the search path makes such code dispatch to
# the stub, which errors -- breaking R CMD check, the pkgdown build, and the
# roxygenize step.
#
# Withholding the `@export` is not enough. `pkgload::load_all()` -- what
# roxygen2 and pkgdown both load the package with -- defaults to
# `export_all = TRUE` and attaches every object in the namespace, `NAMESPACE` or
# no `NAMESPACE`. The name has to be absent from the namespace altogether, so
# the stub is bound under `.dd.<name>` in `R/no-bind.R` instead, away from its
# documentation. `?length` and `dd$length` are unaffected: an Rd topic is named
# by `@name`/`@rdname`, and the `dd` entry points at the internal binding.
#
# This is only needed for names that ordinary R code relies on; the many other
# base-shadowing stubs (`abs()`, `sqrt()`, ...) stay exported as before.
...
code <-
  funs |>
  mutate(
    export_doc = if_else(function_name %in% no_bind, "", "#' @export\n"),
    ...
    # A `no_bind` name that owns its page keeps the full documentation and ends
    # in `NULL` where the other primary blocks assign the stub. One that rides
    # on another name's page needs nothing beyond the objectless block above.
    #
    # The `\usage` goes with the object: `R CMD check` rejects a usage entry for
    # a function the package does not define. `@param` and `@return` stay, so
    # the page still names the arguments, their types and the return type.
    no_bind_primary_roxy = glue(
      r"(
    #' DuckDB function {function_name}
    #'
    #' @description
    ...
    {param_doc}
    #' @return {if_else(return_type == "", "Unspecified.", paste0("`", return_type, "`"))}
    {export_doc}{overloads_doc}{provided_doc}{family_doc}{examples}NULL

    )"
    ),
    roxy = case_when(
      function_name %in% no_object ~ no_object_roxy,
      function_name %in% no_bind & is_primary ~ no_bind_primary_roxy,
      function_name %in% no_bind ~ no_object_roxy,
...
# The `no_bind` stubs live here, in their own file and with no roxygen block, so
# that roxygen2 never sees them: a block on `.dd.length` would alias the help
# page to that name instead of to `length`.
no_bind_code <-
  funs |>
  filter(function_name %in% no_bind) |>
  mutate(
    code = glue(
      r"(
      {tibble:::tick_if_needed(paste0(bind_prefix, function_name))} <- function({signature}) {{
        stop("DuckDB function {function_name}() is not available in R.")
      }}
...
stopifnot(length(no_bind_code) == length(no_bind))
...
invisible(parse(text = no_bind_code))

writeLines(no_bind_code, "R/no-bind.R")
...
# (e.g. `dd$"<->"` is `list_distance`), and the `no_bind` names, whose entry
# points at the internal binding in `R/no-bind.R`.
dd_entries <-
  tibble(
    name = funs$function_name[parsed],
    value = case_when(
      funs$function_name[parsed] %in% no_object ~ funs$rep_name[parsed],
      funs$function_name[parsed] %in% no_bind ~
        paste0(bind_prefix, funs$function_name[parsed]),
      .default = funs$function_name[parsed]
    )
  )
